...
5. If no combinator exists, accept the selector because it only adds a class, ID, attribute, or pseudo-class condition to `.custom-profile-screen` itself.
...
Thorium keeps ownership of the profile-screen selector policy because no maintained JavaScript policy library directly expresses its root-containment rule. CSSOM parses and traverses the stylesheet, while [`css-selector-parser`](https://github.com/mdevils/css-selector-parser) is the selected replacement for the current manual selector traversal. It provides a maintained, TypeScript-compatible selector AST with zero runtime dependencies. Thorium still applies the application-specific allow/deny rules to that AST and rejects the complete document when a selector cannot be proven to remain inside `.custom-profile-screen`. This parser migration is not yet implemented.

The alternatives were [`parsel-js`](https://github.com/LeaVerou/parsel), which is dependency-free but intentionally permissive and supports a narrower selector grammar; PostCSS with `postcss-selector-parser`, which duplicates CSSOM stylesheet parsing and adds a larger runtime dependency surface; Stylelint, which is intended for build-time linting; general CSS sanitizers, whose policies do not enforce Thorium's containment contract; and the Rust [`css-sanitizer`](https://github.com/levish0/css-sanitizer), which is not a JavaScript dependency and still requires a custom selector-isolation policy.
