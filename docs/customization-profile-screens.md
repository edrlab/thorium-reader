...
Thorium keeps the handwritten selector validation in `style.ts` as the runtime reference implementation because no maintained JavaScript policy library directly expresses its root-containment rule. The test suite compares it with two AST-based implementations: [`css-selector-parser`](https://github.com/mdevils/css-selector-parser) and [`parsel-js`](https://github.com/LeaVerou/parsel). Both adapters normalize their parser-specific AST into the same Thorium-owned policy model, and all three implementations run against the complete enforcement corpus. The parser packages are development dependencies and are not included in the production runtime path.
