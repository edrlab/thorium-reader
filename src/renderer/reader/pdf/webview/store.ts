...
export const storeInit = <TStateValue = {}, TCallbackInjection extends {} = {}>(
    ...
    setState: (newState) => (
            ...
            Object.entries(newState)
                .forEach(([key, value]) =>
                    (value as Promise<unknown>).then((v) => state = { ...state, [key]: v })),
            Object.entries(newState)
                .forEach(([key, value]) =>
        ...
        subscribe: (key, fn) =>
            (callbackArray[key as string] =
                (callbackArray[key as string] || new Set()).add(fn), undefined),
        unsubscribe: (fn) => Object.values(callbackArray).forEach((v) => v?.delete(fn)),
        pipe: (key, fn) =>
            (pipeArray[key as string] =
                (pipeArray[key as string] || new Set()).add(fn), undefined),
        unpipe: (fn) => Object.values(pipeArray).forEach((v) => v?.delete(fn)),
    });
