...
import type { IPdfMetadataFallback } from "./manifest";
...
// API
//
export async function pdfPackager(pdfPath: string, metadataFallback?: IPdfMetadataFallback): Promise<string> {
    ...
    const manifest = await pdfManifest(pdfPath, info, metadataFallback);
