...
export async function getSystemProxy(): Promise<ProxyConfig | undefined> {
    if (process.platform === "win32") {
        ...
        if ((proxySettings.ProxyAutoConfigEnable === "1" || proxySettings.ProxyAutoConfigEnable === 1) /* && proxySettings.ProxyAutoDiscoveryEnable === "1" */ && proxySettings.ProxyAutoConfigURLString) {
        ...
        } else if ((proxySettings.HTTPEnable === "1" || proxySettings.HTTPEnable === 1) && proxySettings.HTTPProxy && proxySettings.HTTPPort) {
            return {
                proxyUrl: `http://${proxySettings.HTTPProxy}:${proxySettings.HTTPPort}`,
                noProxy,
            };
        } else if ((proxySettings.SOCKSEnable === "1" || proxySettings.SOCKSEnable === 1) && proxySettings.SOCKSProxy && proxySettings.SOCKSPort) {
            return {
                proxyUrl: `socks://${proxySettings.SOCKSProxy}:${proxySettings.SOCKSPort}`,
                noProxy,
            };
        } else if ((proxySettings.HTTPSEnable === "1" || proxySettings.HTTPSEnable === 1) && proxySettings.HTTPSProxy && proxySettings.HTTPSPort) {
            return {
