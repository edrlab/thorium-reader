...
npm ci --ignore-scripts --foreground-scripts --min-release-age=3 --allow-git=root --cpu=arm64 && cd node_modules/electron && DEBUG=@electron/get* force_no_cache=true npm_config_arch=arm64 ELECTRON_INSTALL_ARCH=arm64 node install.js --cpu=arm64 && cd - &&\
